...
SOURCERS="vars.sh colors.sh"
PREFIX="$HOME/etc/bashparts/"
...
main()
{
    ...
    for file in $SOURCERS ; do
        ...
        [ -r "$fullpath" ] && source $fullpath || echo "[F] Failed to source \"$fullpath\""
        [ -r "$fullpath" ] || echo "[W] Source file \"$fullpath\": not found"
    done
}
